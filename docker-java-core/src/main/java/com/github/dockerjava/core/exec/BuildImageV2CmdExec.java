...
public class BuildImageV2CmdExec extends AbstrAsyncDockerCmdExec<BuildImageV2Cmd, BuildResponseV2Item> implements
        BuildImageV2Cmd.Exec {
    private static final Logger LOGGER = LoggerFactory.getLogger(BuildImageV2CmdExec.class);

    public BuildImageV2CmdExec(WebTarget baseResource, DockerClientConfig dockerClientConfig) {
        super(baseResource, dockerClientConfig);
    }

    private InvocationBuilder resourceWithOptionalAuthConfig(BuildImageV2Cmd command, InvocationBuilder request) {
    ...
    @Override
    protected Void execute0(BuildImageV2Cmd command, ResultCallback<BuildResponseV2Item> resultCallback) {
        ...
        if (command.getOutputs() != null) {
            webTarget = webTarget.queryParam("outputs", command.getOutputs());
        }

        webTarget = webTarget.queryParam("version", command.getVersion() != null ? command.getVersion() : "2");
        ...
        builder.post(new TypeReference<BuildResponseV2Item>() {
